...
int main()
{
    ...
    while (getline(ifile, line)) {
        ...
        if (command1 == "* bfs") {
            ...
            pos = line.find(" ", pos + 1);
            command1 = line.substr(pos + 1, line.length());
            breadthFirstSearch(command1, matrix, cities);
        }
        if (command1 == "* dijkstra") {
            int pos = line.find(" ");
            pos = line.find(" ", pos + 1);
            int pos2 = line.find(" ", pos + 1);
            command1 = line.substr(pos + 1,1);
            command2 = line.substr(pos2+1, line.length());
            dijkstra(command1, command2, matrix, cities);
        }
        cout << endl;
    }
}